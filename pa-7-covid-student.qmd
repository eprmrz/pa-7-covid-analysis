...
author: "Efrain Ramirez and Sam Minard"
format: 
  html: default
  gfm: default
...
state_totals |> 
  ...
  labs(x = "Total Number of Cases",
       y = "State", 
       title = "Covid-19 Cases from January 2020 through March 2023",
       caption = "Source: New York Times, Coronavirus (Covid-19) Data in the United States") +
  theme_light()
...
>Probably not the best way. It would be better to compare proportions of cases by 100,000 people to better compare by state. 
...
state_totals |> 
  ...
  drop_na() |> 
  mutate(case_rate = (total/estimate)*100000) |> 
  arrange(desc(case_rate)) |> 
  slice_head(n = 10) |> 
  ggplot(aes(x = case_rate, 
             y = fct_reorder(state, case_rate))) +
  geom_col(fill = "blue4") +
  labs(x = "Total Number of Cases per 100,000",
       y = "State", 
       title = "Covid-19 Cases from January 2020 through March 2023",
       caption = "Source: New York Times, Coronavirus (Covid-19) Data in the United States") +
  theme_light()
...
>Maybe the NYT data is less reliable than official government sources. Maybe some stuffs underreported compared to others. 
...
>How does the NYT data compare to CDC data? How many individuals in the population had Covid during this time (rather than measuring number cases, where certain people could be counted multiple times because they got it more than once). 
